...
async function pdv() {
    try {
        ...
        infoLogger.info('Arquivos encontrados:' + pdvFiles.length);
        if (pdvFiles.length != 0) {
            for (const file of pdvFiles) {
                infoLogger.info(`Processando arquivo: ${file}`);
                await vendaPdv(`${dirfiles}/${file}`);
            }
            infoLogger.info('Todos os arquivos foram processados.');
        } else {
            infoLogger.info('Nenhum arquivo localizado em: "' + dirfiles + '"');
        }

    } catch (error) {
        errorLogger.error('Erro ao processar arquivos:', error);
    }
}
...
async function getFiles(dirfiles) {
    ...
    if (!fs.existsSync(dirfiles)) {
        errorLogger.error('**************** Diretório inexistente! ', dirfiles, '***********************************')
    } else {
...
async function vendaPdv(file) {
    try {
        warnLogger.warn(`Lendo arquivo: ${file}`);
        ...
        await finalizarProcessamento(registrosINI, registrosDIT, registrosDOC, registrosFIM)
            ...
            .then(() => {
                warnLogger.warn(`Processamento do arquivo ${file} concluído.`);
            })
            .catch((err) => {
                ...
                errorLogger.error('Erro na inclusão do arquivo Djsystem:');
                errorLogger.error(cleanedErrorMessage);
            })
    } catch (err) {
        errorLogger.error(`Erro no arquivo ${file}:`, err);
    }
}
...
async function moveFile(file) {
    ...
    await fs.rename(oldPath, newFilePath, function (err) {
        if (err) throw err
        infoLogger.info('move '+oldPath + ' to ' + newFilePath)
        ...
        firebird.attach(dbOptions, async function (err, db) {
            ...
            db.transaction(firebird.ISOLATION_READ_COMMITTED, async function (err, transaction) {
                ...
                try {
                    ...
                    nextSeq++;
                    infoLogger.info('Proxima sequência de operação = '+nextSeq)
                    ...
                    transaction.commit(function (err) {
                        if (err) {
                            ...
                            infoLogger.info('Transação confirmada');
                            resolve()
                        }
                    });
                    ...
                    // Reverte a transação
                    transaction.rollback(function () {
                        reject(new Error('Erro durante o processamento no arquivo de sequência('+seqquenciaDjsystem+ '): ' + error.message));
                    });
